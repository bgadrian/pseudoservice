...
func (o *GetCustomCountURL) Build() (*url.URL, error) {
	...
	var _path = "/custom/{count}"

	count := swag.FormatInt32(o.Count)
	if count != "" {
		_path = strings.Replace(_path, "{count}", count, -1)
	} else {
		return nil, errors.New("Count is required on GetCustomCountURL")
	}
...
func (o *GetCustomCountURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetCustomCountURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetCustomCountURL")
	}
